...
"""
    ...
    def response(self,query,CV):
        ...
        if(query==""):
            ...
            JD_keyword_prompt=self.keyword_prompt("CV")
            print(JD_keyword_prompt)
            print(self.CV)
            list_topic=self.gpt(JD_keyword_prompt,self.CV)
            print(list_topic)
            self.list_topic=list_topic
        self.query=query
        template=self.QuestionsPrompt("CV")
        # print(template)
...
def Doubt_Solver(request: QuizRequest):
    ...
    if quiz_ID not in generator_CV_Ques:
        generator_CV_Ques[quiz_ID] = Questions(user_ID,user_name)
    if quiz_ID not in generator_CV_Ques:
        raise HTTPException(status_code=500, detail="Failed to create Quiz Bot.")
    output=generator_CV_Ques[quiz_ID].response(user_query,CV)
    # print(output)
